# Axiom Shell Specification (Interaction Layer)

## 1. Overview

The Axiom shell is the primary interaction layer for AxiomOS. It is a high-performance, ergonomic command-line environment and user interface designed for developers and power users. It leverages the underlying system's hybrid architecture and graphics capabilities to provide a "Flow-State" experience.

## 2. Telemetry Integration

The Axiom shell is not just a command runner; it is a real-time system monitor.

- **Real-time Metrics:** CPU usage (specifically P-core vs. E-core utilization), system memory pressure, storage throughput, and thermal data are integrated directly into the shell's status bar or as an overlay.
- **Service Status:** Monitors the health and status of active Plug-in I/O drivers and core Registry services.
- **Hardware Feedback:** Visualizes thread director decisions and thermal throttling events as they occur.

## 3. Signed-Execution Enforcement

To maintain system integrity (as defined in Phase 2), the Axiom shell enforces strict execution policies.
...
The Axiom shell is designed for maximum productivity with minimal distractions.
...
- **Direct Terminal Rendering:** Uses the target platform's graphics subsystem via a native DRM/KMS-like interface for high-resolution, hardware-accelerated terminal rendering. This bypasses legacy VGA/BIOS modes entirely.
- **Integration with VFS:** Direct shell commands for managing pinned files and monitoring storage health, including visibility into the **3-Boot Purge** schedule.
